...
public class Business_Page extends TestBase {
    ...
    @FindBy(xpath = "//h2[text()='Insurance Products']")
    WebElement Insurance_Products;
    @FindBy(xpath = "//h2[text()='Other Products']")
    WebElement Other_Products;
    @FindBy(xpath = "//span[text()='Grid Dashboard']")
    WebElement AP_Grid;
    @FindBy(xpath = "//img[@alt='Turtlemintpro icon']")
    WebElement AP_Grid_logo;
    ...
    public void AP_Grid() throws InterruptedException {

        TestUtil.click(Business, "click On Business Button");
        TestUtil.click(AP_Grid, "Click on AP_Grid");
        WebCommands.staticSleep(4000);
        String  AP_Gridpageurl = driver.getCurrentUrl();
        System.out.println(AP_Gridpageurl + "    AP_Gridpageurl page url");
        Assert.assertEquals(AP_Gridpageurl.contains("summary-screen"), true);
        System.out.println( " landed on ap grid page");
        WebCommands.staticSleep(2000);
        TestUtil.IsDisplayed(AP_Grid_logo,"AP_Grid_logo displayed");
        TestUtil.getScreenShot();
