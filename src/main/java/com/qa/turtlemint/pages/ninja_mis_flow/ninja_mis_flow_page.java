...
public class ninja_mis_flow_page extends TestBase {
    ...
    String Ninjawindow;
    String policy_no = TestUtil.generateRandomPolicyNo(7);
    ...
    WebElement MIS;
    @FindBy(xpath = "//input[@value][@type='text']")
    WebElement mis_searchBar;
    ...
    WebElement policy_status_dropdown;
    @FindBy(xpath = "//div[text()= 'Junk']")
    WebElement junk;
    @FindBy(xpath = "//button[text()= 'Save Sale']")
    WebElement save_sale;
    @FindBy(xpath = "//span[text()= 'Junk']")
    WebElement policy_markedAsJunk;

    @FindBy(xpath = "//div[text()='No results found for ']")
    ...
    public void SelectRegDate() throws InterruptedException {
        ...
        LogUtils.info("closed date picker");
        WebCommands.staticSleep(2000);
  ...
  public void gotIt() {
        try {
            ...
            LogUtils.info("got it Element not present");
        }
  }
    ...
    public void saveninjawindow()  {
        Ninjawindow = driver.getWindowHandle();
    ...
    public void issuedPolicyThroughNinja() throws Exception {
        ...
        Thread.sleep(2000);
        LogUtils.info("switched to ninja");
       ...
       // SearchBar.sendKeys("MHEKWPA86MC");
        ...
        WebCommands.staticSleep(2000);
        TestUtil.sendKeys(enterPolicyNumber,policy_no , "Policy Number Entered");
        System.out.println( "entered policy number " +  policy_no );
        WebCommands.staticSleep(2000);
    ...
    public void Mintprofinalflow() throws Exception {
        ...
        WebCommands.staticSleep(2000);
        TestUtil.getScreenShot();
        WebElement policyInclosedLead=driver.findElement(By.xpath("(//span[contains(text(), '" + regisNo + "')])"));
        TestUtil.IsDisplayed(policyInclosedLead,"policyInclosedLead displayed searched by registerd number");
        LogUtils.info("searched by register number  " + regisNo);
        TestUtil.click(Business, "click On Buisness Button");
        TestUtil.click(InsuranceSales, "Click on Insurance Sale DropDown");
        WebCommands.staticSleep(2000);
        TestUtil.getScreenShot();
        TestUtil.sendKeys(searchbar,policy_no,"searched for lead name siddharth by policy number");
        ...
        WebCommands.staticSleep(2000);
        TestUtil.getScreenShot();
        LogUtils.info("in activity point section Policy should update in cycle 1 if we are in date of 1 to 15 and it should go in cycle 2 if we are in date of 16 to 31 ");
...
//it should go in cycle 2 if we are in date of 16 to 31"
        ...
        driver.switchTo().window(Ninjawindow);
        WebCommands.staticSleep(2000);
        driver.get(prop.getProperty("ninjaurl"));
        LogUtils.info("onto to ninja homepage");
        WebCommands.staticSleep(2000);
        TestUtil.click(MIS, "Click on mis");
        WebCommands.staticSleep(4000);
        TestUtil.sendKeys(mis_searchBar,policy_no + (Keys.RETURN) ,"searched by policy no in mis");
        driver.findElement(By.xpath("(//p[contains(text(), '" + policy_no + "')])")).click();
        WebCommands.staticSleep(2000);
        TestUtil.click(edit_mis, "Click on edit icon");
        WebCommands.staticSleep(2000);
        TestUtil.sendKeys(comment,"testing" ,"comment added");
        TestUtil.click(policy_status_dropdown, "Click on policy_status_dropdown");
        TestUtil.click(junk, "Click on junk");
        TestUtil.click(save_sale, "Click on save_sale");
        TestUtil.IsDisplayed(policy_markedAsJunk,"Junk text displayed");
        driver.switchTo().window(Mintprowindow);
        TestUtil.click(Business, "click On Buisness Button");
        TestUtil.click(InsuranceSales, "Click on Insurance Sale DropDown");
        WebCommands.staticSleep(2000);
        TestUtil.getScreenShot();
        TestUtil.sendKeys(searchbar,policy_no,"searched for lead name siddharth by policy number");
        TestUtil.IsDisplayed(NoResultFound,"Junk policy removed from Insurance_Sales NoResultFound displayed ");
        WebElement JunkpolicyNumber=driver.findElement(By.xpath("(//b[contains(text(), '" + policy_no + "')])"));
        TestUtil.IsDisplayed(JunkpolicyNumber,"JunkpolicyNumber not found msg dispayed");
        TestUtil.getScreenShot();
    }
    }
